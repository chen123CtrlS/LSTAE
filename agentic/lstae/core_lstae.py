...
"""
Core functions to implement , learn from GiGPO.
"""
...
# added for history_step_reward --- for every prompt
def build_history_step_groups(now_obs: str, now_step_score: float, history_obs: np.array, history_step_scores: np.array, similarity_thresh: float = 0.95, summarize: bool = False):
    clusters = []
    history_index = np.array([i for i in range(len(history_obs))])
    step_group_rewards = [now_step_score]
    for obs, loc in zip(history_obs, history_index):
        if are_similar(obs, now_obs, similarity_thresh):
            step_score = history_step_scores[loc]
            step_group_rewards.append(step_score)
            clusters.append({"rep": obs, "step_score": [step_score]})
    step_group_rewards = np.array(step_group_rewards)
    norm_step_group_rewards = step_group_rewards.mean()
    # norm_step_group_rewards = (step_group_rewards - step_group_rewards.mean()) / (step_group_rewards.std() + 1e-8)
    return clusters,norm_step_group_rewards
